    ...
    pub fn insert(
        ...
        if let Some(previous) = self.cur().get(&local) {
            if !is_module || !previous.is_module || fqn != previous.name {
                emit_error(
                    format!("conflicting local name '{}'", local),
                    &[
        ...
        match self.get(&lhs) {
            None => {
                if inbody {
                    if name.len() > 1 {
                        emit_error(
                            format!("possibly undefined name '{}'", lhs),
                            &[(
                                t.loc.clone(),
                                "cannot use :: notation to reference names not tracked by zz",
                            )],
                        );
            ...
            Some(v) => {
                if rhs.len() != 0 && !v.subtypes {
                    emit_error(
                        format!("resolving '{}' as member is not possible", name),
                ...
                */
                ...
                if rhs.len() == 0 && v.is_module {
                    emit_error(
                        format!("cannot use module '{}' as a type", v.name),
                        &[
                            (
                                t.loc.clone(),
                                format!("cannot use module '{}' as a type", name),
                            ),
                            (
                                v.loc.clone(),
                                format!(
                                    "if you wanted to import '{}' as a type, use ::{{{}}} here",
                                    name, name
                                ),
                            ),
                        ],
...
fn check_abs_available(
    ...
    if !fqn.is_absolute() && fqn.len() > 1 {
        emit_warn(
            format!(
                "relative name {} not resolved. likely due to previous error",
                fqn
            ),
    ...
    let module = match all_modules.get(&module_name) {
        None => {
            emit_error(
                format!(
                    "cannot find module '{}' during abs of module '{}'",
    ...
    for local2 in &module.locals {
        if local2.name == local_name {
            if local2.vis == ast::Visibility::Object {
                emit_error(
                    format!("the type '{}' in '{}' is private", local_name, module_name),
                    &[
            ...
            if let Some(local3_as) = &local3_as {
                if local3_as == &local_name {
                    *fqn = Name::from(&format!("{}::{}", importname, local3));
                    return check_abs_available(fqn, this_vis, all_modules, loc, selfname);
                }
            }
    ...
    match stm {
        ...
        ast::Statement::MacroCall {
            ...
            if *scope.complete.borrow() {
                emit_error(
                    format!("macro cannot expand to statements here"),
                    &[(
                        loc.clone(),
                        format!("try wrapping the macro call in ( ) to make it an expression"),
                    )],
                );
            }
            let mut t = ast::Typed {
    ...
    for import in &mut md.imports {
        ...
        if fqn.0[1] == "ext" {
            ...
            if expr.starts_with("\"") && expr.len() > 2 {
                ...
                let path = std::path::Path::new(&import.loc.file)
                    .parent()
                    .expect("ICE: include path resolver")
                    .join(path);
        ...
        let local_module_name = import
            .alias
            .clone()
            .unwrap_or(import.name.0.last().unwrap().clone());
            ...
            if let Some(previous) = ext.ext.lock().unwrap().get(&import.name) {
                if let ast::Def::Include { inline, .. } = previous.def {
                    if inline != import.inline {
                        emit_error(
                            format!("conflicting import modes"),
                            &[
                                (
                                    import.loc.clone(),
                                    format!("{} here", if inline { "inlined" } else { "included" }),
                                ),
                                (
                                    previous.loc.clone(),
                                    format!(
                                        "previously {} here",
                                        if inline { "inlined" } else { "included" }
                                    ),
                                ),
                            ],
                        );
            ...
            if import.inline {
                if !expr.starts_with("\"") || !expr.ends_with("\"") || expr.len() < 3 {
                    emit_error(
                        ...
                        &[(
                            import.loc.clone(),
                            format!("'{}' is not a relative include", expr),
                        )],
                    );
                    std::process::exit(9);
                }
...
fn abs_args(
    ...
    for mut arg in oargs {
        ...
        match &arg.typed.tail {
            ...
            ast::Tail::Dynamic(_) => {
                emit_error(
                    format!("missing tail binding "),
                    &[(
                        arg.loc.clone(),
                        "+ without a name makes no sense in this context",
                    )],
                );
                std::process::exit(9);
            }
            ast::Tail::Static(_, _) => {
                emit_error(
                    ...
                    &[(
                        arg.loc.clone(),
                        "+ with static size makes no sense in this context",
                    )],
                );
                std::process::exit(9);
            }
