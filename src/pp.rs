use super::ast;
use super::name::Name;
use super::parser::{emit_error, emit_warn, Rule};
...
impl PP {
    ...
    pub fn eval(&self, termish: pest::iterators::Pair<'static, Rule>) -> Value {
        ...
        match expr.as_rule() {
            ...
            Rule::pp_call => {
                ...
                match name.0.join("::").as_str() {
                    "feature" => {
                        if args.len() != 1 {
                            emit_error(
                                "wrong number of arguments to feature. expected 1",
                                &[(loc, "called here")],
                            );
                            std::process::exit(9);
                        }
                        ...
                        let s = match &args[0] {
                            ...
                            _ => {
                                emit_error(
                                    "argument to feature must be a string",
                                    &[(loc, "called here")],
                                );
                                std::process::exit(9);
                            }
                        ...
                        match self.features.get(s) {
                            None => {
                                emit_warn(
                                    "undefined feature defaults to false",
                                    &[(
                                        loc,
                                        "avoid this warning by defining it explicitly in zz.toml",
                                    )],
                                );
                                Value::Bool(false)
                            }
                    ...
                    "def" => {
                        if args.len() != 1 {
                            emit_error(
                                "wrong number of arguments to def. expected 1",
                                &[(loc, "called here")],
                            );
                            std::process::exit(9);
                        }
                        ...
                        match &args[0] {
                            ...
                            _ => {
                                emit_warn(
                                    "undefined def defaults to false",
                    ...
                    "target" => {
                        if args.len() != 2 {
                            emit_error(
                                "wrong number of arguments to def. expected 2",
                                &[(loc, "called here")],
                            );
                            std::process::exit(9);
                        }
                        ...
                        match &args[0] {
                            Value::String(s) if s == "asan" => match &args[1] {
                                Value::Bool(s) => Value::Bool(s == &self.stage.asan),
                                _ => {
                                    emit_warn(
                                        "invalid attribute value defaults to false",
                                        &[(loc, "target attribute needs to be a string")],
                                    );
                                    Value::Bool(false)
                                }
                            },
                            Value::String(s) if s == "debug" => match &args[1] {
                                Value::Bool(s) => Value::Bool(s == &self.stage.debug),
                                _ => {
                                    emit_warn(
                                        "invalid attribute value defaults to false",
                                        &[(loc, "target attribute needs to be a string")],
                            ...
                            Value::String(s) => {
                                emit_warn(
                                    "undefined target attribute defaults to false",
                            ...
                            _ => {
                                emit_warn(
                                    "invalid target attribute defaults to false",
                                    &[(loc, "target attribute needs to be a string")],
                                );
                                Value::Bool(false)
                            }
                    ...
                    n => {
                        emit_error(
                            format!("function '{}' not available in preprocessor directive", n),
                            &[(loc, "used here")],
                        );
                        std::process::exit(9);
                    }
            ...
            _ => {
                emit_error(
                    format!(
                        "{:?} expression cannot (yet) be used in preprocessor directive",
                        expr.as_rule()
                    ),
                    &[(loc, "used here")],
                );
    ...
    fn push(&mut self, loc: ast::Location, v: Value) {
        match v {
            ...
            _ => {
                emit_error(
                    "preprocessor directive must evaluate to boolean",
    ...
    fn pop(&mut self, loc: &ast::Location) -> bool {
        if self.stack.len() < 1 {
            emit_error("missing preceeding #if directive", &[(loc.clone(), "here")]);
            std::process::exit(9);
        }
