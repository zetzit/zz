...
pub fn build(buildset: BuildSet, variant: &str, stage: make::Stage, slow: bool) {
    ...
    if root.join("src").exists() {
        loader::load(&mut modules, &project.project, &project_name, &root.join("src"), &features, &stage);
    }
    if root.join("tests").exists() {
        loader::load(&mut modules, &project.project, &project_tests_name, &root.join("tests").canonicalize().unwrap(), &features, &stage);
    }
    ...
    if let Some(deps) = &project.dependencies {
        for (name, dep) in deps {
            match dep {
                toml::Value::String(_) => {
                    getdep(&mut visited, name, &mut modules, &mut project.project, &mut searchpaths, &stage);
                },
                _ => (),
            }
        }
...
fn getdep(
    ...
    if found.join("src").exists() {
        ...
        loader::load(modules, &project.project, &project_name, &found.join("src"), &features, &stage);
    }
