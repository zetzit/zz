...
) -> Result<Module, pest::error::Error<Rule>>
{
    ...
    for decl in decl.next().unwrap().into_inner() {
        match decl.as_rule() {
            ...
            Rule::function | Rule::closure | Rule::theory => {
                let loc = Location::from_span(n.into(), &decl.as_span());
                        ...
                        Rule::fn_args => {
                            ...
                            for arg in part.into_inner() {
                                ...
                                if arg.as_rule() == Rule::vararg {
                                    if declrule != Rule::function {
                                        emit_error(
                                            "vararg not possible here",
                                            &[(argloc, "nope")],
                                        );
                                        std::process::exit(9);
                                    }
                ...
                for part in decl {
                    match part.as_rule() {
                        ...
                        Rule::struct_f => {
                            ...
                            let TypedName { typed, name, tags } =
                                parse_named_type(n, part.next().unwrap());

                            let array = match part.next() {
                                None => Array::None,
                                Some(array) => match array.into_inner().next() {
                                    Some(expr) => Array::Sized(parse_expr(n, expr)),
                                    None => Array::Unsized,
                                },
                            };
                    ...
                    match part.as_rule() {
                        ...
                        Rule::key_shared => {
                            if let Rule::istatic = rule {
                                let e = pest::error::Error::<Rule>::new_from_span(
                                    pest::error::ErrorVariant::CustomError {
                                        message: format!(
                                            "cannot change visibility of static variable"
                                        ),
                                    },
                                    part.as_span(),
                                );
...
pub(crate) fn parse_expr(n: &str, decl: pest::iterators::Pair<'static, Rule>) -> Expression {
    ...
    let climber = PrecClimber::new(vec![
        ...
        //6
        Operator::new(Rule::lessthan, Assoc::Left)
            | Operator::new(Rule::morethan, Assoc::Left)
            | Operator::new(Rule::moreeq, Assoc::Left)
            | Operator::new(Rule::lesseq, Assoc::Left),
        //5
    ...
    let reduce = |lhs: Expression, op: pest::iterators::Pair<'static, Rule>, rhs: Expression| {
        ...
        Expression::Infix {
            ...
            op: match op.as_rule() {
                ...
                _ => {
                    emit_error(
                        format!("ICE: unexpected operator {}", op),
    ...
    match asrule {
        Rule::unarypre => {
            ...
            let op = match part.as_rule() {
                ...
                _ => {
                    emit_error("ICE: unexpected operator", &[(loc.clone(), "in this expr")]);
                    std::process::exit(9);
                }
            };
            let part = expr.next().unwrap();
        ...
        Rule::char_literal => {
            ...
            Expression::LiteralChar { v: v[0], loc }
        }
        Rule::number_literal | Rule::bool_literal => Expression::Literal {
            v: expr.as_str().to_string(),
            loc,
        },
        Rule::expr => parse_expr(n, expr),
        ...
        Rule::macrocall => {
            ...
            Expression::MacroCall { loc, name, args }
        }
        Rule::array_init => {
            let mut fields = Vec::new();
            ...
            Expression::ArrayInit { loc, fields }
        }
        Rule::struct_init => {
            let mut expr = expr.into_inner();
            let part = expr.next().unwrap();
            ...
            Expression::StructInit { loc, typed, fields }
        }
...
pub(crate) fn parse_statement(
    ...
    match stm.as_rule() {
        ...
        Rule::label => {
            ...
            into.push(Box::new(Statement::Label { loc, label }));
        }
        Rule::continue_stm => {
        ...
        Rule::return_stm => {
            ...
            match key.as_rule() {
                Rule::key_return => {}
                a => {
                    panic!("expected key_return instead of {:?}", a);
                }
            };
            ...
            match *current_if_statement {
                None => {
                    emit_error(
                        ...
                        &[(
                            loc.clone(),
                            "this else branch does not follow an if condition",
                        )],
                    );
                    std::process::exit(9);
                }
        ...
        Rule::macrocall => {
            ...
            into.push(Box::new(Statement::MacroCall { loc, name, args }));
        }
...
pub(crate) fn parse_block(
    ...
    match decl.as_rule() {
        Rule::block => {}
        Rule::macro_expanded_to_statements => {}
        _ => {
            panic!("parse_block called with {:?}", decl);
        }
    };
...
pub(crate) fn parse_named_type(n: &str, decl: pest::iterators::Pair<'static, Rule>) -> TypedName {
    match decl.as_rule() {
        Rule::named_type => {}
        _ => {
            panic!("parse_named_type called with {:?}", decl);
        }
    };
    ...
    let name = match name_part.as_rule() {
        Rule::ident => {
            ...
            if name == "return" {
                ...
                emit_error("syntax error", &[(loc, "llegal use of keyword 'return'")]);
                std::process::exit(9);
            }
            name
        }
        _ => {
            let loc = Location::from_span(n.into(), &name_part.as_span());
            emit_error("syntax error", &[(loc.clone(), "expected a name")]);
            std::process::exit(9);
        }
    };
...
pub(crate) fn parse_anon_type(n: &str, decl: pest::iterators::Pair<'static, Rule>) -> Typed {
    match decl.as_rule() {
        Rule::anon_type => {}
        _ => {
            panic!("parse_anon_type called with {:?}", decl);
        }
    };
    let loc = Location::from_span(n.into(), &decl.as_span());
    //the actual type name is always on the left hand side
...
pub(crate) fn parse_importname(
    decl: pest::iterators::Pair<Rule>,
) -> (Name, Vec<(String, Option<String>)>) {
                    ...
                    match p2.as_rule() {
                        Rule::local_i => {
                            ...
                            let name = match name.as_rule() {
                                Rule::ident => name.as_str().to_string(),
                                Rule::qident => {
        ...
        match part.as_rule() {
            Rule::call_args => {
                args = part
                    .into_inner()
                    .into_iter()
                    .map(|arg| Box::new(parse_expr(n, arg)))
                    .collect();
            }
